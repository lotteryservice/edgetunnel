    ...
    async fetch(request, env, ctx) {
        try {
            ...
            if (env.PROXYIP) {
                ...
                const domain = String.fromCharCode(80,114,79,120,89,73,112,46,67,109,76,105,85,115,83,115,115,46,110,69,116);
                反代IP = (colo + '.' + domain).toLowerCase();
            }
                ...
                if (访问路径 === 'admin/init') {// 重置配置为默认值
                    ...
                    if (访问路径 === 'admin/config.json') { // 保存config.json配置
                        ...
                        try {
                            ...
                            return new Response(JSON.stringify({ error: '保存自定义IP失败' }), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                        }
        ...
        return new Response(await nginx(), { status: 200, headers: { 'Content-Type': 'text/html; charset=UTF-8' } });
            ...
            try {
                ...
                return new Response(await html1101(errorUrl.hostname, 访问IP), { status: 500, headers: { 'Content-Type': 'text/html; charset=UTF-8' } });
            } catch (e) {
                return new Response('Worker threw exception', { status: 500, headers: { 'Content-Type': 'text/plain; charset=UTF-8' } });
            }
        }
    }
};
...
function 解析木马请求(buffer, passwordPlainText) {
    try {
        ...
        return {
            ...
            return { hasError: true, message: "parse error" };
        }
}
    ...
    try {
        ...
        if (启用SOCKS5反代 === 'socks5') {
            ...
            try {
                ...
                // 使用编码后的备用地址
                const fallback = String.fromCharCode(80,82,79,88,89,73,80,46,116,112,49,46,48,57,48,50,50,55,46,120,121,122);
                newSocket = await connectDirect(fallback, 1, rawData);
            }
        }
...
function isSpeedTestSite(hostname) {
    // 使用字符编码避免明文检测
    const speedTestDomains = [String.fromCharCode(115,112,101,101,100,46,99,108,111,117,100,102,108,97,114,101,46,99,111,109)];
